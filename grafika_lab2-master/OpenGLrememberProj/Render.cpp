...
bool lightMode = true;
bool textureReplace = true;		//Эта переменная отвечает за смену текстур
...
void keyDownEvent(OpenGL *ogl, int key)
{
	...
	if (key == 'E')											//Смена текстуры происходит по этой кнопке
	{
...
void Render(OpenGL *ogl)
{
	...
	for (double i = -0.648; i <= 0.355; i += 0.0001)
	{
		double x = 8.5 + 6.70820393 / 2 * cos(i * 3.141593);
		double y = 7 + 6.70820393 / 2 * sin(i * 3.141593);

		double tx = tx0 + 1.0 / 9500.0;
		double ty = ty0 + 1.0 / 9500.0;
		...
		Det[0] = { x - (8.5 + 6.70820393 / 2 * cos((i - 0.0001) * 3.141593)) };
		Det[1] = { y - (7 + 6.70820393 / 2 * sin((i - 0.0001) * 3.141593)) };
